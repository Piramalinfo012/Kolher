    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 md:p-6 bg-neutral-950/85 backdrop-blur-md overflow-y-auto">
        <motion.div
          ...
          className="bg-[#FAFAFA] dark:bg-neutral-900 rounded-3xl shadow-2xl max-w-6xl w-full border border-neutral-200 dark:border-neutral-800 overflow-hidden flex flex-col max-h-[94vh]"
          id="dynamic-product-customizer-dialog"
        >
          {/* ================= TOP HEADER BAR ================= */}
          <div className="px-6 py-4 bg-white dark:bg-neutral-900 border-b border-neutral-200/80 dark:border-neutral-800 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-4">
              <div>
                <span className="font-mono text-xs font-bold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider block">
                  {product.model_number}
                </span>
                <h1 className="text-xl sm:text-2xl font-serif text-neutral-900 dark:text-white font-bold tracking-tight">
                  {product.product_name}
                </h1>
                <p className="text-xs text-neutral-500 dark:text-neutral-400 font-light italic">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`p-2.5 rounded-full border transition-colors ${
                  isFavorited
                    ? 'border-rose-300 bg-rose-50 text-rose-600 dark:bg-rose-950/60 dark:border-rose-800'
                    : 'border-neutral-200 dark:border-neutral-700 hover:border-neutral-300 bg-white dark:bg-neutral-800 text-neutral-400 dark:text-neutral-300 hover:text-rose-500'
                }`}
              >
              ...
              <button
                type="button"
                onClick={onClose}
                className="p-2.5 rounded-full border border-neutral-200 dark:border-neutral-700 hover:border-neutral-300 bg-white dark:bg-neutral-800 text-neutral-500 dark:text-neutral-300 hover:text-neutral-900 dark:hover:text-white transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
          ...
          <div className="flex-1 overflow-y-auto grid grid-cols-1 lg:grid-cols-12 min-h-0">
            ...
            <div className="lg:col-span-5 xl:col-span-4 p-6 space-y-8 overflow-y-auto bg-white dark:bg-neutral-900 border-r border-neutral-200/80 dark:border-neutral-800">
              
              {effectiveParts.map((part, index) => (
                <div key={part.id} className="space-y-3.5">
                  {index !== 0 && <hr className="border-neutral-200 dark:border-neutral-800" />}
                  
                  <div className="flex items-center justify-between">
                    <h3 className="text-xs font-bold uppercase tracking-wider text-neutral-900 dark:text-white font-sans">
                      {part.name}
                    </h3>
                    {selectedOptions[part.id] && (
                      <span className="text-[11px] font-medium text-red-900 dark:text-red-300 bg-red-50 dark:bg-red-950/60 px-2.5 py-0.5 rounded-full border border-red-200 dark:border-red-800">
                  ...
                  <div className="grid grid-cols-4 sm:grid-cols-5 gap-3 pt-1">
                    {part.options.map(opt => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`w-12 h-12 rounded-full transition-all relative flex items-center justify-center shadow-sm overflow-hidden border border-neutral-100 dark:border-neutral-800 ${
                              isSelected ? 'ring-2 ring-red-500 scale-105' : 'hover:scale-105 hover:shadow-md'
                            }`}
                          ...
                          <span className={`mt-2 text-[9px] font-medium tracking-wide uppercase leading-tight max-w-[75px] line-clamp-3 ${isSelected ? 'text-red-600 dark:text-red-400 font-bold' : 'text-neutral-500 dark:text-neutral-400'}`}>
                            {opt.name}
                          </span>
                        </button>
                      );
              ...
              <div className="pt-6 border-t border-neutral-200 dark:border-neutral-800">
                ...
                <input
                  ...
                  className="w-full text-xs px-3.5 py-2.5 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-800 text-neutral-900 dark:text-white focus:bg-white dark:focus:bg-neutral-800 focus:outline-none focus:ring-2 focus:ring-red-500/30 focus:border-red-500"
                />
              </div>
            </div>

            {/* RIGHT COLUMN: VISUALIZER STAGE */}
            <div className="lg:col-span-7 xl:col-span-8 bg-gradient-to-b from-[#FAF8F5] via-[#F4F3EF] to-[#ECEAE4] dark:from-neutral-950 dark:via-neutral-900 dark:to-neutral-950 p-6 flex flex-col justify-between relative">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] uppercase font-bold tracking-widest text-neutral-600 dark:text-neutral-300 bg-white/80 dark:bg-neutral-900/80 border border-neutral-200/80 dark:border-neutral-800 px-2.5 py-1 rounded-full backdrop-blur-xs flex items-center gap-1.5 shadow-2xs">
                  <Sparkles className="w-3 h-3 text-red-600 dark:text-red-400" />
                  Bespoke Configuration
                </span>
                <div className="flex items-center gap-1.5 bg-white/80 dark:bg-neutral-900/80 border border-neutral-200/80 dark:border-neutral-800 p-0.5 rounded-xl shadow-2xs">
                  <button
                    type="button"
                    onClick={() => setViewMode('PHOTO')}
                    className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all cursor-pointer ${
                      viewMode === 'PHOTO' ? 'bg-neutral-900 dark:bg-neutral-800 text-white shadow-2xs' : 'text-neutral-500 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white'
                    }`}
                  >
                    Photo Studio
                  </button>
                  {has3D && (
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all cursor-pointer ${
                        viewMode === '3D' ? 'bg-neutral-900 dark:bg-neutral-800 text-white shadow-2xs' : 'text-neutral-500 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white'
                      }`}
                    >
                      3D Model
                    </button>
              ...
              <div className="bg-white/90 dark:bg-neutral-900/90 backdrop-blur-md rounded-2xl p-4 border border-neutral-200/80 dark:border-neutral-800 shadow-xs space-y-2 mt-2">
                <div className="flex items-center justify-between text-xs text-neutral-500 dark:text-neutral-400 font-medium">
                  ...
                  <span className="font-mono text-neutral-800 dark:text-neutral-200">₹{basePrice.toLocaleString('en-IN')}</span>
                </div>
                {(() => {
                  const entries = Object.entries(selectedOptions) as [string, CustomizationOption][];
                  return entries.map(([catId, opt]) => {
                    ...
                    if (opt.price_modifier > 0) {
                      return (
                        <div key={catId} className="flex items-center justify-between text-xs text-neutral-500 dark:text-neutral-400 font-medium">
                          <span>{cat?.name} ({opt.name}):</span>
                          <span className="font-mono text-red-700 dark:text-red-400">+ ₹{opt.price_modifier.toLocaleString('en-IN')}</span>
                        </div>
                      );
                    }
                    return null;
                ...
                <div className="pt-2 border-t border-neutral-200 dark:border-neutral-800 flex items-center justify-between text-xs font-bold text-neutral-900 dark:text-white">
                  <span>Unit MRP (Excl. Taxes):</span>
                  <span className="font-mono text-neutral-900 dark:text-white text-sm">₹{unitFinalPrice.toLocaleString('en-IN')}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* ================= BOTTOM BAR ================= */}
          <div className="px-6 py-4 bg-white dark:bg-neutral-900 border-t border-neutral-200 dark:border-neutral-800 flex flex-wrap items-center justify-between gap-4 shrink-0 shadow-lg">
            <div className="flex items-center gap-3">
              <span className="text-xs font-bold uppercase tracking-wider text-neutral-600 dark:text-neutral-300">
                Quantity:
              </span>
              <div className="flex items-center border border-neutral-200 dark:border-neutral-700 rounded-xl bg-white dark:bg-neutral-800 overflow-hidden shadow-2xs">
                <button
                  type="button"
                  onClick={() => setQuantity(Math.max(1, quantity - 1))}
                  className="p-2 hover:bg-neutral-100 dark:hover:bg-neutral-700 text-neutral-600 dark:text-neutral-300 transition-colors"
                >
                  <Minus className="w-3.5 h-3.5" />
                </button>
                <span className="px-4 text-xs font-bold font-mono text-neutral-900 dark:text-white min-w-[36px] text-center">
                  {quantity}
                </span>
                <button
                  type="button"
            ...
            <div className="flex items-center gap-6">
              <div className="text-right">
                ...
                <div className="text-xl sm:text-2xl font-bold font-serif text-neutral-950 dark:text-white">
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={onClose}
                  className="px-4 py-3 rounded-xl border border-neutral-200 dark:border-neutral-700 hover:bg-neutral-100 dark:hover:bg-neutral-800 text-xs font-bold text-neutral-700 dark:text-neutral-300 transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-6 py-3 rounded-xl bg-neutral-900 dark:bg-neutral-800 hover:bg-neutral-800 dark:hover:bg-neutral-700 text-white font-bold text-xs sm:text-sm shadow-md flex items-center gap-2 transition-all cursor-pointer"
                >
